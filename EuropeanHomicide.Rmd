...
```{r renaming}
# I use the rename() function in the dplyr package:
...
```
Now the column is renamed to 'homicides'.
I can check whether it worked by using the colnames() function:
```{r checking-if-renaming-worked}
...
```{r making-line-graphs}
# I use geom_line() to make a line graph instead:
...
# The facet layout is already two columns and three rows(?)
# I can make it three columns and two rows by changing nrow = 3 to nrow = 2:
...
```{r changing-legend}
# First, I rename the "Entity" column "Country":
Western_Europe_renamed_again <- Western_Europe_renamed %>% 
  rename(
    Country = Entity
  )

# Then I make a ggplot with this new renamed data:
...
# I have positioned the legend at the bottom by using theme(legend.position = "bottom")
```
